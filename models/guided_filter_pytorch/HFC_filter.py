...
class HFCFilter(nn.Module):
    def __init__(self, filter_width=23, nsig=20, ratio=4, sub_low_ratio=1, sub_mask=False, is_clamp=True):
        ...
        self.gaussian_filter = Gaussian_kernel(filter_width, nsig=nsig)
...
def get_kernel(kernel_len=16, nsig=10):
    GaussianKernel = cv2.getGaussianKernel(kernel_len, nsig) * cv2.getGaussianKernel(kernel_len, nsig).T
...
class Gaussian_kernel(nn.Module):
    def __init__(self,
        ...
        kernel = get_kernel(kernel_len=kernel_len, nsig=nsig)
