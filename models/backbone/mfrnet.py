...
class UFProGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        unet_block8 = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, norm_layer=norm_layer, innermost=True)  # 最内层
        ...
        unet_block1 = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, outermost=True, norm_layer=norm_layer)  # 最外层
        ...
        self.splitFre = splitFrequencyModule(in_c=3, basic_dim=ngf)
    ...
    def forward(self, input, encode_only=False):
        ...
        if encode_only:  # just use the encoder
            return d6  # [1, ngf*8, 4, 4]

        d7 = self.down7(d6)  # [1, ngf*8, 2, 2]
        d8 = self.down8(d7)  # [1, ngf*8, 1, 1]

        low_part, out_high = self.splitFre(img)  # [1, ngf, 256, 256]  [1, ngf, 256, 256]
        ...
        return u1, d6  # return image and deep feature
        # return u1
...
class lowFrequencyPromptFusion(nn.Module):
    ...
    def forward(self, feature, prompt_feature):
        ...
        key_value = key_value.reshape(b, -1, 2, self.num_heads, c1 // self.num_heads).permute(2, 0, 3, 1,
                                                                                              4).contiguous()
...
class dynamic_filter_channel(nn.Module):
    def __init__(self, inchannels, kernel_size=3, stride=1, group=8):
        ...
        self.bn = nn.BatchNorm2d(group * kernel_size ** 2)  # 归一化

        self.act = nn.Softmax(dim=-2)
        nn.init.kaiming_normal_(self.conv.weight, mode='fan_out', nonlinearity='relu')  # 初始化 self.conv 层的权重
        self.pad = nn.ReflectionPad2d(kernel_size // 2)
        self.ap_1 = nn.AdaptiveAvgPool2d((1, 1))  # 自适应平均池化，将输入特征图的空间尺寸压缩为1x1
    ...
    def forward(self, x):
        ...
        low_filter = low_filter * self.act_gate(self.conv_gate(low_filter))  # 动态调整
        ...
        low_filter = self.act(low_filter)  # [1,8,1,9,1]

        full_filter = torch.ones_like(low_filter)  # 生成一个与低频滤波器形状相同的全1滤波器
        high_filter = full_filter - low_filter  # 计算高频滤波器
        high_filter = self.act(high_filter)  # 使用Softmax激活函数进行归一化

        low_part = torch.sum(x * low_filter, dim=3).reshape(n, c, h, w)

        out_high = torch.sum(x * high_filter, dim=3).reshape(n, c, h, w)  # 计算高频部分
        out_high = identity_input - low_part
        return low_part, out_high

# 频率提示生成器
class frequencyPromptGeneration(nn.Module):
...
class H_LGate(nn.Module):
    ...
    def forward(self, x):
        max = torch.max(x,1,keepdim=True)[0]  # 通道维度上计算最大值
        mean = torch.mean(x,1,keepdim=True)  # 通道维度上计算平均值
        scale = torch.cat((max, mean), dim=1)
        scale = self.spatial(scale)  # 7x7 conv
        scale = F.sigmoid(scale)  # sigmoid 激活
...
class L_HGate(nn.Module):
    def __init__(self, dim):
        ...
        self.avg = nn.AdaptiveAvgPool2d((1,1))  # 自适应平均池化
        self.max = nn.AdaptiveMaxPool2d((1,1))  # 自适应最大池化
...
class FreRefine(nn.Module):
    ...
    def forward(self, low, high):
        H_L_weight = self.H_L_Gate(high)  # 计算高频到低频的权重
        L_H_weight = self.L_H_Gate(low)  # 计算低频到高频的权重
...
class PromptModule(nn.Module):
    def __init__(self, basic_dim=32, dim=32, input_resolution=128):
        ...
        self.FPG_high = frequencyPromptGeneration(basic_dim, h, w, flag_highF=True)
        self.FPG_low = frequencyPromptGeneration(basic_dim, h, w, flag_highF=False)
...
class splitFrequencyModule(nn.Module):
    ...
    def forward(self, F_low):
        ...
        F_low = self.patch_embed(F_low)  # [1, 128, 256, 256]
