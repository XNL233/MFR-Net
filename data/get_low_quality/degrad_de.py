...
from eyelash_simulation import eyelash_simulation
...
def DE_COLOR(img, brightness=0.3, contrast=0.4, saturation=0.4):
    ...
    img = transform(img)
    img = img.numpy()
    color_params = {'brightness_factor': brightness_factor, 'contrast_factor': contrast_factor,
...
def DE_HALO(img, h, w, brightness_factor, center=None, radius=None):
    ...
    halo_params = {'num': num, 'center_a': center_a, 'dia_a': dia_a, 'center_b': center_b, 'dia_b': dia_b}
...
def DE_ILLUMINATION(img, region_mask, h=512, w=512, brightness=0.3, contrast=0.4, saturation=0.4, d_min=0.4, d_max=0.7):
    ...
    illum_params = {'color': color_params, 'halo': halo_params, 'hole': hole_params}
...
def DE_SPOT(img, h, w, center=None, radius=None, s_max_num=10):
    ...
    for i in range(s_num):
        ...
        spot_dict = {'radius': radius, 'center': center, 'rad_w': rad_w, 'rad_h': rad_h, 'sigma': sigma}
...
def DE_BLUR(img, h, w, center=None, radius=None, ratio=1):
    ...
    blur_params = {'sigma': sigma, 'rad_w': rad_w, 'rad_h': rad_h}
...
def DE_process(img, mask, h, w, de_type):
    ...
    # New eyelash-like artifacts simulation
    elif de_type == '1000':
        img = np.array(img)
        img = eyelash_simulation(img, mask, (h, w))
    elif de_type == '1001':
        img, illum_params = DE_ILLUMINATION(img, mask, h, w, brightness, contrast, saturation, d_min, d_max)
        params['illumination'] = illum_params
...
if __name__ == '__main__':
    img = Image.open('../../images/hq/1.png').convert('RGB')  #
    h,w = img.size
    mask = Image.open('../../images/hq/mask/1.png').convert('L')
    mask = np.expand_dims(mask, axis=2)
    mask = np.array(mask, np.float32).transpose(2, 0, 1) / 255.0

    result,_ = DE_ILLUMINATION(img, mask, h, w, )
    result = (np.transpose(result*mask, (1, 2, 0)) * 255).astype(np.uint8)

    imwrite('../../images/hq/final.png', result)
    print("Finshed")
