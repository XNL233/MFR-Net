...
def imread(file_path, c=None):
    if c is None:
        ...
        im = cv2.imread(file_path, c)  # c = cv2.IMREAD_GRAYSCALE 读取灰度图像
    if im is None:
        raise 'Can not read image'
    if im.ndim == 3 and im.shape[2] == 3:  # 图像维度为 3, 且通道数为 3
...
def _get_center_radius_by_hough(mask):
    circles = cv2.HoughCircles((mask * 255).astype(np.uint8), cv2.HOUGH_GRADIENT, 1, 1000, param1=5, param2=5,
                               minRadius=min(mask.shape) // 4, maxRadius=max(mask.shape) // 2 + 1)
    center = circles[0, 0, :2]  # 圆心坐标 [x, y]
    radius = circles[0, 0, 2]  # 圆的半径
